...
def solve_CCG(
    ...
    while True:
        ...
        for eps_bar in eps_bar_list:
            ...
            # Split facilities into active (undisrupted) and disrupted.
            # For disrupted facilities eps_scalar[j]==0, so all recourse
            # variables are zero and all associated constraints are trivially
            # satisfied — we skip them entirely to keep the master small.
            ...
            # Only add variables for undisrupted (active) facilities
            for j in J_active:
                for r in R:
            ...
            # Opt-cut: only active facilities contribute revenue/congestion
            master.addConstr(
            ...
            # Customer assignment constraints: only over active facilities
            for i in I:
                master.addConstr(
            ...
            # Facility constraints: only for active facilities
            for j in J_active:
                master.addConstr(
            ...
            # SOC congestion: only for active facilities
            for j in J_active:
        ...
        # MIP warm start: inject previous solution to help Gurobi prune B&B tree
        for j in J:
            for r in R:
                x[j, r].Start = x0[j, r]
